...
function mgstyle_pagination(){ global $wp_query; $big=999999999; $links=paginate_links(['base'=>str_replace($big,'%#%',esc_url(get_pagenum_link($big))),'format'=>'?paged=%#%','current'=>max(1,get_query_var('paged')),'total'=>$wp_query->max_num_pages,'type'=>'array','prev_text'=>'«','next_text'=>'»']); if($links){ echo '<nav class="pagination" role="navigation" aria-label="Posts Pagination">'; foreach($links as $l) echo $l; echo '</nav>'; } }
